...
q  = '''
SELECT MAX(date_time_msk) as l_dt FROM `EXTERNAL_DATA_SOURCES.CALLTOUCH_JOURNAL`
'''
...
if len(new_rows) >0 :
    ...
    query_job = client.query(
        f"""
        DELETE
        FROM `EXTERNAL_DATA_SOURCES.CALLTOUCH_JOURNAL`
        WHERE date_time_msk >= '{start_date}'
        """
    )
    results = query_job.result()
    new_rows.to_gbq(f'EXTERNAL_DATA_SOURCES.CALLTOUCH_JOURNAL', project_id='m2-main', if_exists='append', credentials=gbq_credential)